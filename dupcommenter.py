...
This script does the following:
  ...
  • It then checks every open issue versus all older issues (open or closed) and computes 
    ...
    threshold, it is flagged as potentially a duplicate.
  • For any open issue that appears to be a duplicate of one or more earlier issues, the script
    posts a comment on GitHub listing the duplicate issue numbers.
...
if not GITHUB_TOKEN:
    raise RuntimeError("You must set the GITHUB_TOKEN environment variable with appropriate scopes")

# GitHub API base URL
GITHUB_API = "https://api.github.com"
# A threshold for duplicate similarity
...
class GitHubClient:
        ...
        while True:
            ...
            if response.status_code == 403:
                ...
                if "X-RateLimit-Remaining" in response.headers and response.headers["X-RateLimit-Remaining"] == "0":
                    reset_time = int(response.headers.get("X-RateLimit-Reset", time.time() + RATE_LIMIT_SLEEP))
                    sleep_for = max(reset_time - int(time.time()), RATE_LIMIT_SLEEP)
                    logging.warning("Rate limit reached. Sleeping for %d seconds...", sleep_for)
                    time.sleep(sleep_for)
                    continue
                else:
                    logging.warning("403 received. Retrying in %d seconds...", RATE_LIMIT_SLEEP)
    ...
    def add_comment(self, repo, issue_number, comment_text):
        ...
        if response.ok:
            logging.info("Added comment on issue #%d: %s", issue_number, comment)
        else:
            logging.error("Failed to add comment on issue #%d: %s", issue_number, response.text)
        return response
...
def detect_duplicates(issues_db):
    ...
    c.execute("SELECT number, title, body, state, created_at, duplicate_commented FROM issues ORDER BY created_at ASC")
    rows = c.fetchall()
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Deduplicate GitHub issues.")
    parser.add_argument("repo", help="GitHub repository in the form 'owner/repo'")
    ...
    for issue in gh.get_issues(repo, state="all", since=since):
        ...
        if not issue_exists(conn, issue["number"]):
            logging.info("Storing new issue #%d: %s", issue["number"], issue.get("title", "").strip())
    ...
    if not dup_results:
        ...
        for open_issue_num, dup_nums in dup_results:
            ...
            comment = f"Potentially a duplicate of issue(s): {dup_list_text}"
            logging.info("Issue #%d appears duplicate of: %s", open_issue_num, dup_list_text)
            # Post the comment
